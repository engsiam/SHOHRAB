...
export default function Hero() {
  ...
  return (
    <section ref={heroRef} className="min-h-screen flex flex-col items-center justify-center pt-32 px-gutter relative overflow-hidden" id="home">
      <div className="absolute inset-0 z-0 bg-[radial-gradient(circle_at_50%_50%,rgba(0,219,231,0.05),transparent_70%)]"></div>
      ...
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-12 gap-12 items-center relative z-10">
        <div ref={textRef} className="md:col-span-7">
          ...
          <div className="flex gap-6 items-center hero-text-item">
            ...
            <a className="text-on-surface hover:text-primary-fixed transition-colors flex items-center gap-2" href="https://github.com/yourusername" target="_blank" rel="noopener noreferrer">
              <Github className="w-5 h-5" />
              <span className="text-xs font-bold uppercase tracking-widest">GitHub</span>
            </a>
            <a className="text-on-surface hover:text-primary-fixed transition-colors flex items-center gap-2" href="https://linkedin.com/in/yourusername" target="_blank" rel="noopener noreferrer">
        ...
        {/* Floating Tech Icons */}
        <div ref={(el) => { floatingRefs.current[0] = el; }} className="absolute -top-6 -right-6 w-14 h-14 rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 flex flex-col items-center justify-center shadow-lg animate-bounce duration-[3000ms] z-20">
          <Server className="w-6 h-6 text-primary-fixed" />
          <span className="text-[8px] font-bold text-primary-fixed leading-none text-center uppercase tracking-tighter mt-1">Node.js</span>
        </div>
